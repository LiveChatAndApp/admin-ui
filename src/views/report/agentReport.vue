  ...
  <div class="app-container calendar-list-container">
    ...
    <div class="filter-container">
      ...
      <el-table :data="list" v-loading="paging.isLoading" :element-loading-text="$t('table.loading')"
        ...
        <el-table-column :label="$t('agentReport.amount')">
          <template slot-scope="scope">{{scope.row.amount | money}}</template>
        </el-table-column>
        <!-- <el-table-column :label="$t('agentReport.rateDifference')">
          <template slot-scope="scope">{{scope.row.rateDifference}}</template>
        </el-table-column>
        <el-table-column :label="$t('agentReport.rateFixedAmountDifference')">
          <template slot-scope="scope">{{scope.row.rateFixedAmountDifference}}</template>
        </el-table-column>
        <el-table-column :label="$t('agentReport.feeDifference')">
...
<script>
  ...
  import { exportAgentReportListCsv, getAgentReportList } from '@/api/report'
  ...
  export default {
    ...
    methods: {
      getList() {
        ...
        getAgentReportList(this.getQueryParams()).then(response => {
          this.list = response.content

          if (this.list.length > 0) {
            const summary = response.summary
            summary.successTime = this.$t('table.total')
      ...
      handleExport() {
        exportAgentReportListCsv(this.getQueryParams())
      },
      getQueryParams() {
        const params = {
          ...this.paging.query
